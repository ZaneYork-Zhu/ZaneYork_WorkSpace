 ...
 * 
 * @param  sendData: 待发送的单个字节数据。 
 * @param  receiveData: 用于存储接收到的数据的指针。 
 *                     如果为NULL，则使用临时变量占位，不实际存储接收数据。 
 * @return HARDWARE_SPI_State: SPI操作的状态，可能是HARDWARE_SPI_FINISH（操作完成） 
 *                             或HARDWARE_SPI_ERROR（操作出错）。 
 */ 
HARDWARE_SPI_State flash_spiHardwareSwapByte(uint8_t sendData, uint8_t *receiveData) 
{ 
    // 定义一个HAL状态结构体变量，并初始化为HAL_OK 
    HAL_StatusTypeDef HAL_StatusTypeStruct = HAL_OK; 
    uint8_t dummyReceiveData; 
    // 如果不需要接收数据，使用一个临时变量占位 
    uint8_t *receiveData_buf = (receiveData != NULL) ? receiveData : &dummyReceiveData; 
    // 检查SPI句柄是否有效 
    ...
    // 根据不同的SPI通信方式进行数据传输 
        ...
        HAL_StatusTypeStruct = HAL_SPI_TransmitReceive(&gFlash_SPI_HandleTypeSturct, &sendData, receiveData_buf, 1, HARDWARE_SPI_SWAP_BYTE_TIMEOUT); 
    #endif 
 
    #if _HARDWARE_SPI_DMA 
        // 使用DMA方式实现SPI通信 
        // 调用HAL库的SPI DMA发送函数进行数据传输 
        HAL_StatusTypeStruct = HAL_SPI_TransmitReceive_DMA(&gFlash_SPI_HandleTypeSturct, &sendData,receiveData_buf, 1); 
        // 等待DMA传输完成 
    ...
    #if _HARDWARE_SPI_IT 
        // 使用中断方式实现SPI通信 
        // 调用HAL库的SPI中断发送函数进行数据传输 
        HAL_StatusTypeStruct = HAL_SPI_Transmit_IT(&gFlash_SPI_HandleTypeSturct, &sendData, 1); 
        // 等待数据传输完成 
        while (HAL_SPI_GetState(&gFlash_SPI_HandleTypeSturct) != HAL_SPI_STATE_READY) 
        { 
            // 等待数据传输完成 
