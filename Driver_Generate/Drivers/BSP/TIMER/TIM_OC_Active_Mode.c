#include "TIM_OC_Active_Mode.h"
#include <stdio.h>

TIM_HandleTypeDef g_TIM_OC_Active_Handle = {0};
...
/**
 * @brief  【不影响产生匹配标志位】
 * @note 1:TIM_OCMODE_ACTIVE模式->"匹配有效模式"[匹配成功->输出有效电平]
 *          1`TIM_OCPOLARITY_HIGH[有效高电平] ->[类似于<延时[匹配值累加]输出高电平>]
 *              无效电平'低电平'->[达到匹配值]->输出有效电平'高电平' 
 *          2`TIM_OCPOLARITY_LOW[有效低电平] ->[类似于<单脉冲->一段高电平脉冲[匹配值累加过程]>]
 *              无效电平'高电平'->[达到匹配值]->输出有效电平'低电平'
 *       2:TIM_OCMODE_INACTIVE模式->"匹配无效模式"[匹配成功->输出无效电平]
 *          1`TIM_OCPOLARITY_HIGH[有效高电平] ->
 *              有效电平'高电平'->[达到匹配值]->输出无效电平'低电平'
 *          2`TIM_OCPOLARITY_LOW[有效低电平] ->
 *              有效电平'低电平'->[达到匹配值]->输出无效电平'高电平'
 */

TIM_OC_InitTypeDef g_TIM_OC_Channel_Struct = {
#if TIM_OC_ACTIVE_MODE_ENABLE
    .OCMode = TIM_OCMODE_ACTIVE,                        // 输出比较模式：匹配有效模式
#else
    .OCMode = TIM_OCMODE_INACTIVE,                      // 输出比较模式：匹配无效模式
...
void TIM_OC_MODE_Test(void)
{
    ...
    TIM_OC_Active_Mode_Init(&g_TIM_OC_Base_Struct, &g_TIM_OC_Channel_Struct);
    ...
    while (1)
    {
        ...
        // 轮询输出比较
        if (__HAL_TIM_GET_FLAG(&g_TIM_OC_Active_Handle, TIM_FLAG_CC1))
        {
            __HAL_TIM_CLEAR_FLAG(&g_TIM_OC_Active_Handle, TIM_FLAG_CC1); // 清除输出比较通道1标志位
            printf("TIM OC Event,CNT_Value = %d CCR_Value =  %d !!\r\n", 
                __HAL_TIM_GET_COUNTER(&g_TIM_OC_Active_Handle), 
                __HAL_TIM_GET_COMPARE(&g_TIM_OC_Active_Handle, TIM_CHANNEL_1));
        }
...
void TIM_OC_Active_Mode_Init(NEW_TIM_HandleTypeDef *pBaseStr, TIM_OC_InitTypeDef *pOCInitStr)
{
    g_TIM_OC_Active_Handle.Instance = pBaseStr->HAL_Handle.Instance; // 定时器实例
    g_TIM_OC_Active_Handle.Init.Period = pBaseStr->HAL_Handle.Init.Period; // 自动重载寄存器值
    g_TIM_OC_Active_Handle.Init.Prescaler = pBaseStr->HAL_Handle.Init.Prescaler; // 预分频系数
    g_TIM_OC_Active_Handle.Init.ClockDivision = pBaseStr->HAL_Handle.Init.ClockDivision; // 时钟分频系数
    g_TIM_OC_Active_Handle.Init.CounterMode = pBaseStr->HAL_Handle.Init.CounterMode; // 计数模式
    g_TIM_OC_Active_Handle.Init.RepetitionCounter = pBaseStr->HAL_Handle.Init.RepetitionCounter; // 重复计数值
    if(HAL_TIM_OC_Init(&g_TIM_OC_Active_Handle) != HAL_OK)
    {        
    ...
    if(HAL_TIM_OC_ConfigChannel(&g_TIM_OC_Active_Handle, &g_sConfigOC_Handle, pBaseStr->OCchannel) != HAL_OK)
    {        
...
#if TIM_OC_MODE_POLLING_ENABLE
    // 启动定时器输出比较轮询
    HAL_TIM_OC_Start(&g_TIM_OC_Active_Handle, pBaseStr->OCchannel);
#endif /* TIM_OC_MODE_POLLING_ENABLE */

#if TIM_OC_MODE_IT_ENABLE
    // 启动定时器输出比较中断
    HAL_TIM_OC_Start_IT(&g_TIM_OC_Active_Handle, pBaseStr->OCchannel);
#endif /* TIM_OC_MODE_IT_ENABLE */

#if TIM_OC_MODE_DMA_ENABLE
    // 启动定时器输出比较DMA
    if(HAL_TIM_OC_Start_DMA(&g_TIM_OC_Active_Handle, pBaseStr->OCchannel, g_DMA_CCRBuffer, sizeof(g_DMA_CCRBuffer) / sizeof(g_DMA_CCRBuffer[0])) != HAL_OK)
    {
        printf("TIM OC Start DMA Error!\n");
        return;
    }
    __HAL_TIM_ENABLE_IT(&g_TIM_OC_Active_Handle, TIM_IT_CC1); // 使能输出比较通道1中断
