...
#if _HARDWARE_SPI_ENABLE
    /*初始化hardware SPI*/
    spiHardwareInit(0,_HARDWARE_SPI,SPI_BAUDRATEPRESCALER_256);
...
FLASH_State _W25Q128_ReadID(JEDEC_ID *driverId)
{
    ...
    {
        /*从 FLASH 芯片中发送读取ID号【指令】*/ 
        spiState = spiHardwareSwapByte(FlashJedecDeviceId, NULL); // 传递地址 
        printf("Device spiState[1] = %d \r\n", (uint8_t) spiState); 
        /*从 FLASH 芯片中接收<厂商>ID号【数据】*/ 
        spiState = spiHardwareSwapByte(Flash_DUMMY_BYTE, &driverId->manuFacturerId); // 传递地址 
        printf("Device spiState[2] = %d \r\n", (uint8_t) spiState); 
        /*从 FLASH 芯片中接收内存类型*/ 
        spiState = spiHardwareSwapByte(Flash_DUMMY_BYTE, &driverId->memoryType); // 传递地址 
        printf("Device spiState[3] = %d \r\n", (uint8_t) spiState); 
        /*从 FLASH 芯片中接收容量标识*/ 
        spiState = spiHardwareSwapByte(Flash_DUMMY_BYTE, &driverId->capacityId); // 传递地址 
...
static FLASH_State _W25Q128_writeEnable(void)
{
    ...
    {
        // 使能写入操作 
        spiState = spiHardwareSwapByte(FlashWriteEnable,NULL);
    }
    /*恢复SPI空闲*/
    _HARDWARE_SPI_CS(1);
    if(spiState != HARDWARE_SPI_FINISH)
...
static FLASH_State _W25Q128_waitBusy(void)
{
    ...
    {
        // 读取状态寄存器1 
        spiState = spiHardwareSwapByte(FlashReadStatusReg1,NULL);
        // 获取当前BUSY的状态
        // 循环检查BUSY位，直到空闲或超时
        do{
            // 再次发送DUMMY字节读取最新状态
            spiState = spiHardwareSwapByte(Flash_DUMMY_BYTE,&waitBusyData);
            if (spiState != HARDWARE_SPI_FINISH)
            {
                _HARDWARE_SPI_CS(1);
                return FLASH_ERROR;
...
{ 
    ...
    HARDWARE_SPI_State spiState = HARDWARE_SPI_FINISH;
    /* 发送 bit23 ~ bit16 地址 */
    spiState = spiHardwareSwapByte((uint8_t)(address>> 16),NULL);
    /* 发送 bit15 ~ bit8  地址 */ 
    spiState = spiHardwareSwapByte((uint8_t)(address >> 8),NULL); 
    /* 发送 bit7  ~ bit0  地址 */
    spiState = spiHardwareSwapByte((uint8_t)(address),NULL);    
    if(spiState != HARDWARE_SPI_FINISH)
    {
        printf("Device sendAddress ERROR !\r\n");
        return FLASH_ERROR;
...
FLASH_State _W25Q128_onePageWrite(uint32_t pageStartAddress,uint8_t *dataArray,uint8_t dataLen)
{
    ...
    {
        // 1·页编程指令 
        spiState = spiHardwareSwapByte(FlashPageProgram,NULL);
        // 2·发送页地址
        flashState = sendAddress(pageStartAddress);
        // 3·写入数据
        for(uint16_t len = 0;len<dataLen;len++)
        {
            spiState = spiHardwareSwapByte(dataArray[len],NULL);
        }
    }
    _HARDWARE_SPI_CS(1);
    /*3.等待flash传输完成*/
...
FLASH_State _W25Q128_eraseSector(uint16_t eraseSectorAddr)
{
    ...
    {
        // 1.扇区擦除指令
        spiState = spiHardwareSwapByte(FlashSectorErase,NULL);
        // 2·发送物理实际地址
        flashState = sendAddress(eraseAddr);
    }
    _HARDWARE_SPI_CS(1);
...
FLASH_State _W25Q128_eraseBlock(uint16_t eraseBlockAddr)
{
    ...
    {
        // 1.扇区擦除指令
        spiState = spiHardwareSwapByte(FlashBlockErase,NULL);
        // 2·发送物理实际地址
        flashState = sendAddress(blockAddr);
    }
    _HARDWARE_SPI_CS(1);
...
FLASH_State _W25Q128_eraseChip(void)
{
    ...
    _HARDWARE_SPI_CS(0);
    {
        spiState = spiHardwareSwapByte(FlashChipErase,NULL);  /* 发送读寄存器命令 */
    } 
    _HARDWARE_SPI_CS(1);
    flashState = _W25Q128_waitBusy();
    if(spiState != HARDWARE_SPI_FINISH || flashState != FLASH_OK)
...
{ 
    ...
    {
        // 1·标准数据读取
        spiState = spiHardwareSwapByte(FlashReadData,NULL);
        // 2·发送待读取地址
        flashState = sendAddress(StartAddress);
        // 3·读取数据
        for(uint32_t len = 0;len<dataLen;len++)
        {
            spiState = spiHardwareSwapByte(Flash_DUMMY_BYTE,tempData++);
        }
    }
    _HARDWARE_SPI_CS(1);
    if(spiState != HARDWARE_SPI_FINISH || flashState != FLASH_OK)
