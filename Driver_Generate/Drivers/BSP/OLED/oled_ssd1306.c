...
/**
  ...
  */
uint8_t gOLED_gram_buffer[PAGE_MAX][COL_MAX] = {0};

//uint8_t text[]= {};
void OLED_SSD1306_Init_text(void)
{
    OLED_SSD1306_Init();
    uint8_t Image[] = { 0xFF,0x01,0xE1,0x11,0x49,0x25,0x45,0x05,0x45,0x25,0x49,0x11,0xE1,0x01,0xFF,
 ...
 *        该函数首先初始化I2C软件，然后遍历初始化命令序列，依次发送每个命令及其参数到OLED显示屏， 
 *        以完成对显示屏的各项参数设置，使其进入正常显示模式。 
 * @param 无 
 * @retval 无 
 */ 
...
#endif
#if IIC_HARDWARE_ENABLE 
    HAL_StatusTypeDef hal_status = HAL_I2C_Master_Transmit(&hi2c1, OLED_ADRESS, sendbuffer, 2, 1000); 
...
#if IIC_HARDWARE_ENABLE 
    // 使用硬件 I2C 发送数据 
    HAL_StatusTypeDef hal_status = HAL_I2C_Master_Transmit(&hi2c1, OLED_ADRESS, (uint8_t *)sendbuffer, Cont_len + 1, 1000); 
    if (hal_status == HAL_OK) { 
        // 若硬件发送成功，将操作结果设置为成功 
        Operation_state = OP_SUCCESS; 
    } else { 
...
/** 
 * @brief 向 OLED 发送命令 
 * @param write_command 要发送的命令指针 
 * @param Cont_len 要发送的命令长度 
 * @retval OP_SUCCESS 发送成功；OP_FAILURE 发送失败 
 */ 
static OperationResult OLED_WriteCont_Command(const uint8_t *write_command,uint8_t Cont_len){
    return OLED_Cont_Write(write_command, WRITE_COMMAND_MODE,Cont_len); 
}
/**
...
OperationResult OLED_UpdateToGRAM_ALL(void)
{
	...
	for (uint8_t page_index = 0; page_index < PAGE_MAX; page_index++){
        /*设置光标位置为每一页的第一列*/
		operation_state = OLED_SetCursor(page_index, 0);
...
OperationResult oLED_Show_offset_Image(uint8_t start_y, uint8_t start_x, uint8_t image_width, uint8_t image_height, const uint8_t *Img) { 
    ...
    // 保存清除操作的结果到 state 变量。 
    OperationResult state = oLED_Clear_Area(start_x, start_y, image_width, image_height); 
    // 如果清除操作失败，直接返回该结果。 
