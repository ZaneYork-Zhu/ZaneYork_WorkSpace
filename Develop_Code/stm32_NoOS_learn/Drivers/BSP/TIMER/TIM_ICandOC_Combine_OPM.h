#ifndef __TIM_ICANDOC_COMBINE_OPM_H_
#define __TIM_ICANDOC_COMBINE_OPM_H_
...
#define TIMx_OC_CHx_Pull                          GPIO_PULLDOWN
...
#define TIMx_IC_CHx_Pin                           GPIO_PIN_1
...
#define __HAL_RCC_GPIO_IC_CHx_CLK_ENABLE()        do {__HAL_RCC_GPIOA_CLK_ENABLE();} while(0)
...
#define TIM_OC_OPM_MODE_ENABLE                   (1) // 0: 禁止输出比较翻转模式使能 ,1: 允许输出比较翻转模式使能
...
typedef struct new_TIM_OnePulse_InitTypeDef{
    TIM_OnePulse_InitTypeDef TIM_OnePulse_Handle; // 定时器单脉冲模式初始化结构体
    uint32_t OnePulseMode; // 单脉冲模式
    uint32_t OCchannel; // 输出比较通道
    uint32_t ICchannel; // 输入捕获通道
}new_TIM_OnePulse_InitTypeDef;
...
extern void TIM_OC_OPM_Mode_Init(TIM_HandleTypeDef *pBaseStr, new_TIM_OnePulse_InitTypeDef *pOPMInitStr);
...
#endif /* __TIM_ICANDOC_COMBINE_OPM_H_ */
