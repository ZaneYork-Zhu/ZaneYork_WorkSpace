...
OperationResult HAL_OledDeviceFlushGramForDisplay(DisplayDevice *ptDisplayDevice)
{
	...
	for (uint8_t page_index = 0; page_index < PAGE_MAX; page_index++){
        /*设置光标位置为每一页的第一列*/
		operation_state = OLED_Cont_SetCursor(page_index, 0);
        operation_state = OLED_WriteCont_Data((const uint8_t*)ptDisplayDevice->GramAddress[page_index],COL_MAX);
        //printf("page_index:%d\r\n",page_index);
        // 如果设置光标位置失败，返回失败结果
        if(operation_state !=OP_SUCCESS)
        {
            ptDisplayDevice->OperationState = OP_FAILURE;
            return OP_FAILURE;
        }
    }
    // 所有操作都成功，更新操作状态并返回成功结果 
    ptDisplayDevice->OperationState = OP_SUCCESS;
    return OP_SUCCESS;
}

/** 
 * @brief 在指定坐标点亮或熄灭像素 
 * @param ptDisplayDevice 显示设备指针 
 * @param x 显示列位置，使用绝对坐标 (0, 0) 作为参考 
 * @param y 显示行位置，使用绝对坐标 (0, 0) 作为参考 
 * @param isLit 像素状态，1 表示点亮，0 表示熄灭 
...
OperationResult HAL_OledDeviceSetPixel(DisplayDevice *ptDisplayDevice, uint8_t x, uint8_t y, uint8_t isLit) { 
    ...
    // 计算像素在页内的偏移量 
    uint8_t bit_offset = x % PAGE_SIZE; 
 
    // 步骤 2: 越界检查 
    // 检查坐标是否超出显示区域的边界 
    if (x >= ptDisplayDevice->ptData->iX || page >= (ptDisplayDevice->ptData->iY / PAGE_SIZE)) { 
        return OP_FAILURE; 
    } 
 
    // 步骤 3: 根据 isLit 值设置像素状态 
    if (isLit) { 
        // 点亮像素：将对应位设置为 1 
        ((unsigned char (*)[COL_MAX])ptDisplayDevice->GramAddress)[page][x] |= (1 << bit_offset); 
    } else { 
        // 熄灭像素：将对应位设置为 0 
        ((unsigned char (*)[COL_MAX])ptDisplayDevice->GramAddress)[page][x] &= ~(1 << bit_offset); 
    } 
 
    // 操作成功完成，返回 OP_SUCCESS 
    return OP_SUCCESS; 
} 
